...
// backup rotation, and atomic writes for the `oaskills` CLI.
//
...
// install / uninstall / status flows can stay thin: pure path and
...
//   - Plugin normalization: convert legacy object form `{ "<name>": <value> }`
//     to an array of base names before install/uninstall dedup.
 ...
 */
export const resolveConfigDir = (env: NodeJS.ProcessEnv): string => {
 ...
 * variant).
 * Non-string entries return false.
 */
 ...
 * entries are removed entirely so the install flow can append one fresh entry
...
export const dedupePlugins = (entries: readonly string[]): string[] => {
  // Strip all plugin entries first — they will be re-added by the caller
  // with the requested version. This guarantees at most one plugin entry
  ...
  // "last occurrence wins" falls out naturally.
 ...
 * `"opencode-agent-skills-md@latest"` when no version is supplied, otherwise
 ...
 */
export const loadGlobalConfig = (fs: CliFs, env: NodeJS.ProcessEnv): LoadedConfig => {
