// ---------------------------------------------------------------------------
// src/cli/install.ts — `oaskills install` command.
//
// Idempotent install for `opencode-agent-skills-md[@version]`. Existing
// plugin entries are removed before the new specifier is appended so
...
export interface InstallOptions {
  /** Optional version pin (e.g. `"1.2.3"`, `"latest"`). Omit for bare `@latest`. */
  version?: string;
...
/**
 * Run `oaskills install` against the global OpenCode config.
 *
...
export const runInstall = (
  ...
  const nonPlugin = existing.filter((entry) => !matchesPlugin(entry));
  ...
  console.log(`tip: Run \`oaskills doctor\` to verify your setup`);
