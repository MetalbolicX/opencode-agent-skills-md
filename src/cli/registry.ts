...
// Used by `status`, `doctor`, and `update` — keeping the logic here means
// every consumer agrees on what "stale" means and how to degrade when the
// registry is unreachable.
//
// Design notes:
//   - `fetchLatestVersion()` uses Node 20+ global `fetch` directly.
//     A short `AbortSignal.timeout` keeps a flaky network from stalling
//     `oaskills doctor` / `oaskills status`.
//   - `getInstalledVersion()` reuses a source/bundled path-probe pattern
//     to find the right `package.json` whether running from `src/` or
//     bundled into `dist/`.
...
/**
 * Hit `https://registry.npmjs.org/opencode-agent-skills-md/latest` and return
...
export const fetchLatestVersion = async (
  ...
  try {
    ...
    })) as unknown as { ok?: boolean; json(): Promise<unknown> } | undefined;
