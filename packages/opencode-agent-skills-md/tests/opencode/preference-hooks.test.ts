...
import {
  ...
  PREFERENCE_TOOL_IDS,
} from "../../src/preference-hooks";

/**
 * Preference-hook unit tests.
 *
 * Pins the contract for the two hook helpers that back Layers 1 and 2
 * of the preference layer:
 *
 *   - `applySystemTransform` pushes the `<skill-preference-policy>`
 *     block to `output.system`, leaves existing entries alone, and
 *     short-circuits when the env var disables the layer.
 *   - `applyToolDefinition` appends a one-sentence note to
 *     `output.description` only when `toolID` is in `PREFERENCE_TOOL_IDS`
 *     and the layer is enabled. Tools outside the set pass through
 *     untouched (no parameter mutation).
 *
 * The env-var gate (`OPENCODE_AGENT_SKILLS_PREFERENCE_MODE=off`)
 * disables both helpers, matching the spec's "off disables every
 * layer" scenario.
 */
...
describe("applySystemTransform", () => {
  ...
  test("appends the policy block to output.system when enabled", () => {
    ...
    const output: SystemTransformOutput = { system: ["<existing-system>"] };
  ...
  test("does nothing when the env var is the literal 'off'", () => {
    ...
    const output: SystemTransformOutput = { system: ["<existing>"] };
  ...
  test("recovers gracefully when output.system is missing (defensive push target)", () => {
    const output = {} as SystemTransformOutput;
...
describe("applyToolDefinition", () => {
  ...
  test("appends the note to description when toolID is in PREFERENCE_TOOL_IDS", () => {
    const input: ToolDefinitionInput = { toolID: "read" };
    const output: ToolDefinitionOutput = {
      description: "Read a file from disk.",
      parameters: { type: "object" },
    };

    const annotated = applyToolDefinition(input, output);
  ...
  test("does not annotate tools outside PREFERENCE_TOOL_IDS", () => {
    const input: ToolDefinitionInput = { toolID: "todowrite" };
    const output: ToolDefinitionOutput = {
  ...
  test("does not annotate when input.toolID is missing or not a string", () => {
    ...
    assert.equal(
      applyToolDefinition({} as ToolDefinitionInput, output),
      false,
      "empty input is ignored",
    );
    assert.equal(
      applyToolDefinition({ toolID: undefined } as unknown as ToolDefinitionInput, output),
      false,
  ...
  test("recovers gracefully when output.description is missing", () => {
    const input: ToolDefinitionInput = { toolID: "edit" };
    const output = { parameters: { type: "object" } } as ToolDefinitionOutput;
  ...
  test("annotates every documented native tool in the set", () => {
    ...
    for (const toolID of PREFERENCE_TOOL_IDS) {
      const fresh: ToolDefinitionOutput = {
        description: "base",
        parameters: {},
      };
      const annotated = applyToolDefinition({ toolID }, fresh);
